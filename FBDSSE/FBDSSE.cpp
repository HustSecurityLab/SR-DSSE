...
void Update(Client &client, unordered_map<string,CTelement> &Server)
{
    ...
    mpz_t bs;
    mpz_init2(bs, MAX_FILE_LEN);               //random operation a and operation b
    mpz_urandomb(bs,grt,MAX_FILE_LEN);

	string in = rand_str(8);
	...
	mpz_init(ct.Num);
	mpz_add(ct.Num, bs, sk);
	mpz_mod(ct.Num, ct.Num, modnum);

	// cout<<"The size of cipher is: "<<mpz_size(ct.Num)<<endl;
...
void Search(Client &myclient, unordered_map<string,CTelement> &myserver)
{
	...
	for(int j = count; j>=0;j--)
	{
		...
		mpz_add(result,result,myserver[UT].Num);
		mpz_mod(result,result,modnum);
	...
	for(int i = count;i>=0;i--)
	{
		...
		mpz_add(Sum_sk, Sum_sk, sk_i);
		mpz_mod(Sum_sk, Sum_sk, modnum);
	}

	mpz_sub(result, result, Sum_sk);
	mpz_mod(result, result, modnum);

	// cout<<"size of cipher from server: "<<sizeofservercipher<<endl;
...
int main()
{
	...
	string word = "resurg";   //Get keyword from Erone data
